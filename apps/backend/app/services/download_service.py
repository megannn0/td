...
def sanitize_filename(name: str, max_length: int = 200) -> str:
    """Sanitize a filename for safe use on the local filesystem.
    
    Removes or replaces characters that are invalid on Windows/Linux,
    such as newlines, colons, angle brackets, double quotes, slashes,
    pipes, question marks, asterisks, and trailing dots/spaces.
    Also truncates to max_length to avoid path length issues.
    ...
    max_length: int
        Maximum length of the sanitized filename (excluding extension).
    ...
    if len(sanitized) > max_length:
        name_part, dot, ext = sanitized.rpartition('.')
        if dot:
            # Truncate name part, keep extension
            keep = max_length - len(ext) - 1
            sanitized = name_part[:max(0, keep)] + '.' + ext
        else:
            sanitized = sanitized[:max_length]
    # Fallback if empty
    if not sanitized or sanitized in ('.', '..'):
        sanitized = f"file_{abs(hash(name))}"
    return sanitized
...
async def start_download(
    ...
    *,
    ...
    for part in parts:
        ...
        # Ensure started_at is timezone-aware to avoid subtraction errors
