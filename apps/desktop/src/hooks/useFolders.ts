...
import { REFRESH_FOLDERS, REFRESH_ALL, dispatchRefresh } from '../utils/events';
...
export function useFolders(parentId: number | null): UseFoldersReturn {
  ...
  useEffect(() => {
    ...
    window.addEventListener(REFRESH_FOLDERS, handler);
    window.addEventListener(REFRESH_ALL, handler);
    return () => {
      window.removeEventListener(REFRESH_FOLDERS, handler);
  ...
  const create = useCallback(
    async (name: string, parent: number | null) => {
      try {
        ...
        await fetchFolders();
        dispatchRefresh(REFRESH_FOLDERS);
        return folder;
      } catch (err) {
        console.error(err);
        return null;
